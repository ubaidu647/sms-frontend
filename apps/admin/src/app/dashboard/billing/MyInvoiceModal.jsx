...
import { fmtMoney, fmtDate, INVOICE_STATUS_COLORS } from './format';
...
// action — paying invoices is super-admin only.
export default function MyInvoiceModal({ isOpen, onClose, invoice }) {
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Invoice"
      subtitle={invoice?.serialNumber ? `#${invoice.serialNumber}` : undefined}
      size="lg"
      closeOnBackdrop
    >
      {!invoice ? (
        <p className="py-8 text-center text-gray-500 dark:text-gray-400">No invoice selected.</p>
      ) : (
        <div className="space-y-5">
          ...
          {invoice.status === 'unpaid' && (
            <p className="rounded-lg bg-amber-50 dark:bg-amber-950/30 p-3 text-sm text-amber-800 dark:text-amber-300">
