...
export default function RegenerateSectionModal({ isOpen, onClose }) {
  ...
  const { data: classData } = useQuery({
    queryKey: ['classes-regen', effectiveBranchId, academicYear],
    queryFn: () =>
      fetchData({
        url: '/class/list',
        page: 1,
  ...
  const { data: sectionData } = useQuery({
    queryKey: ['sections-regen', classId],
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      const msg = err.message || 'Failed to regenerate section';
      setSubmitError(msg);
      toast.error(msg);
    },
  });
  ...
  const handleSubmit = () => {
    ...
    const payload = { sectionId, month, reason: reason.trim() };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Regenerate Section Vouchers"
      subtitle="For every enrolled student in the section: regenerate unpaid vouchers, generate missing ones, skip those with payments"
      size="lg"
      footer={
      ...
      {!result ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              placeholder="Fee structure correction"
              className={inputCls}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-4 gap-3 text-center">
            ...
            <Stat
              label="Skipped"
              value={(result.skippedPaymentCount || 0) + (result.skippedOtherCount || 0)}
