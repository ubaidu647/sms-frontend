...
export default function AddStaffModal({ open, onClose, canCreateAllBranch, userBranchId }) {
  const C = useColors();
  const { mutate, isPending } = useCreateStaff({ onSuccess: () => onClose() });
    ...
    >
      <SafeAreaView style={[styles.safe, { backgroundColor: C.card }]} edges={['top', 'bottom']}>
        <View style={[styles.header, { borderBottomColor: C.border }]}>
          <View style={{ flex: 1 }}>
            <Text style={[styles.title, { color: C.text }]}>Add Staff</Text>
