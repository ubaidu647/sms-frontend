    ...
    <div className="space-y-4">
      ...
      {!isFetching && count === 0 && (
        <div className="bg-white rounded-xl border border-green-200 p-8 text-center">
          ...
          <p className="text-green-700 font-medium mt-3">All sections have marked attendance for {date}.</p>
        </div>
      )}
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Academic Year</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Strength</th>
                </tr>
              </thead>
              <tbody>
                {sections.map((s) => (
                  <tr key={s.sectionId} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div className="text-sm font-medium text-gray-900">{s.class?.name || '—'}</div>
                      {s.class?.grade && (
                        <div className="text-xs text-gray-400">Grade {s.class.grade}</div>
                      )}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">{s.sectionName}</td>
