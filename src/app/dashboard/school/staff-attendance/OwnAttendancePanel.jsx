...
export default function OwnAttendancePanel() {
  ...
  const { data, isFetching, error } = useQuery({
    queryKey: ['own-staff-attendance', staffId, from, to],
    queryFn: () =>
      fetchData({
        url: `/staff-attendance/staff/${staffId}/history`,
        token,
        from,
        to,
      }),
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <input
            ...
            className="text-sm bg-white px-3 py-2 rounded-lg border border-gray-200 text-gray-900 outline-none"
          />
      ...
      {isFetching ? (
        ...
        <div className="text-sm text-gray-500">No attendance records in this range.</div>
      ) : (
              ...
              {rows.map((r) => (
                <tr key={r._id} className="border-b border-gray-100">
                  ...
                  <td className="py-2 pr-4 text-gray-700">
                    {r.arrivalTime || r.checkIn || '—'}
                  </td>
                  <td className="py-2 pr-4 text-gray-700">
                    {r.departureTime || r.checkOut || '—'}
                  </td>
                  <td className="py-2 pr-4 text-gray-700">
