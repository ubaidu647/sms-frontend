        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-gray-50 p-4">
              <div className="text-xs text-gray-500 uppercase tracking-wide">Total Reads</div>
              <div className="text-3xl font-bold text-gray-900 mt-1">{stats.totalReads ?? 0}</div>
            </div>
            {stats.requiresAck && (
          ...
          <div>
            ...
            {reads.length === 0 ? (
              <p className="text-sm text-gray-500">Nobody has read this yet.</p>
            ) : (
              <div className="overflow-x-auto rounded-lg border border-gray-200 max-h-96 overflow-y-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {reads.map((r) => (
                      <tr key={r._id || r.userId} className="border-t border-gray-100">
                        ...
                        <td className="px-3 py-2 text-gray-600">{formatDateTime(r.readAt)}</td>
                        {stats.requiresAck && (
